...
class SelfIslandDataServices {
    ...
    static func createSelfIsland(island: SelfIsland, _ completion: @escaping (_ error: Error?) -> Void)  {
        ...
        let blockForExecutionInBackground: BlockOperation = BlockOperation(block: {
            ...
            self.getFirstSelfIsland { (error, selfIsland) in
                if (error != nil) {
    ...
    /// Function responsible for getting all self islands
    /// - parameters:
    ///     - completion: closure to be executed at the end of this method
    /// - throws: if an error occurs during getting an object from database (Errors.DatabaseFailure)
    static func getAllSelfIslands(_ completion: ((_ error: Error?, _ selfIslands: [SelfIsland]?) -> Void)?) {
        // block to be executed in background
        let blockForExecutionInBackground: BlockOperation = BlockOperation(block: {
            // error to be returned in case of failure
            var raisedError: Error? = nil
